...
flist = [('AX_2006_2007','AX',5,'AX'),
         ...
         ('KENE_2003_2004','KECK',5,'KENE'),
         ('KENE_2004_2005','KECK',5,'KENE'),
         ('KENE_2005_2006','KECK',5,'KENE'),
...
for f in np.arange(len(flist)):
    ...
    df['d'] = [df.dettime[i].day for i in df.index] # add df column for day    
...
#        thisgroup = df[(df['m']==monthyear[m][1]) & (df['y']==monthyear[m][0])]
        ...
        ndays = len(np.unique(thisgroup.d)) # number of days with calls        
...
#        plt.plot(x,y,'ro')                    
        ...
        # get kurtosis of peaks - possible discriminator of good/bad peaks
        k_ipi = []
        k_freq = []
        ...
        keepthese = (k_ipi>0) & (k_freq >0) & (peak_counts > 50)
        ipi_coords = ipi_coords[keepthese]
        freq_coords = freq_coords[keepthese]
        prop_counts = prop_counts[keepthese]
        peak_counts = peak_counts[keepthese]
        ...
        # number of days with call sequences in current month
        all_ndays = np.ones(len(peak_counts))*ndays
        ...
        for pdex in np.arange(len(prop_counts)):
            ...
            ipiwidthlims = ipi_centers_dense[ipi_zcross]
            ipimean = np.mean(ipisub[(ipisub>=ipiwidthlims[0]) & (ipisub<=ipiwidthlims[1])])
            ipi_std[pdex] = (ipiwidthlims[-1]-ipiwidthlims[0])/2.355
...
#                ipimean = np.mean(ipisub[(ipisub>=ipiwidthlims[0]) & (ipisub<=ipiwidthlims[1])])
#                ipi_std[pdex] = (ipiwidthlims[-1]-ipiwidthlims[0])/2.355
#                #ipi_std[pdex] = np.std(ipisub[(ipisub>=ipiwidthlims[0]) & (ipisub<=ipiwidthlims[1])])
...
#                peak_counts[pdex] = 0
            ...
            freqwidthlims = freq_centers_dense[freq_zcross]
            freqmean = np.mean(fsub[(fsub>=freqwidthlims[0]) & (fsub<=freqwidthlims[1])])
            freq_std[pdex] = (freqwidthlims[-1]-freqwidthlims[0])/2.355
...
#                freqmean = np.mean(fsub[(fsub>=freqwidthlims[0]) & (fsub<=freqwidthlims[1])])
#                freq_std[pdex] = (freqwidthlims[-1]-freqwidthlims[0])/2.355
#                #freq_std[pdex] = np.std(fsub[(fsub>=freqwidthlims[0]) & (fsub<=freqwidthlims[1])])
...
#        
        ...
        allpeaksdat = np.transpose(
            np.array([stavec,netvec,datevec,freq_coords,freq_std,ipi_coords,ipi_std,peak_counts,prop_counts,all_nseq,all_ndays]))
        cols = ['station','netvec','datevec','freq','freqsd','ipi','ipisd','peakcounts','propcounts','all_nseq','all_ndays']
...
#peaksdf.to_csv('ALL_seq_summary_6nov2016_5dBthresh_kurtosis.csv',sep=',')
