...
df['ipi_SE'] = df['ipisd']/np.sqrt(df['all_ndays'])
df['freq_SE'] = df['freqsd']/np.sqrt(df['all_ndays'])
...
df_B = df_decade[(df_decade.freq>=21)]     
df_A = df_decade[(df_decade.freq < 21)] 
...
df_main = df_A[(df_A['ipi'] > 20)] # Singlet, A type
...
ax2.set_xticklabels([])
ax2.scatter(df_B['year'],df_B['ipi'],s=100,
            linewidths=0.5,alpha=0.7,
            c=df_B['stacolors'],edgecolors='none')
ax2.scatter(df_A['year'],df_A['ipi'],s=170,
            linewidths=0.5,alpha=0.7,
            c=df_A['stacolors'],edgecolors='none',marker='*')
...
intercept = "%02i" %mod_ipi.params[0]
...
ax3.set_xlabel('Year')
ax3.scatter(df_B['year'],df_B['freq'],s=100,
            linewidths=0.5,alpha=0.7,
            c=df_B['stacolors'],edgecolors='none')
ax3.scatter(df_A['year'],df_A['freq'],s=170,
            linewidths=0.5,alpha=0.7,
            c=df_A['stacolors'],edgecolors='none',marker='*')
...
intercept = "%02i" %mod_freq.params[0]
...
### More line fitting for doublet call types

df_DA = df_A[(df_A['ipi'] < 20) & (df_A['year']>=2008)] # Doublet, A type
mod_ipi_DA= smf.ols('ipi~year',data=df_DA).fit()
mod_freq_DA = smf.ols('freq~year',data=df_DA).fit()

df_DB = df_B[(df_B['ipi'] < 20) & (df_B['year']>=2008)] # Doublet, B type
# Drop the known outlier
df_DB = df_DB.drop(df_DB.index[df_DB.index==69])
mod_ipi_DB= smf.ols('ipi~year',data=df_DB).fit()
mod_freq_DB = smf.ols('freq~year',data=df_DB).fit()


df_SB = df_B[(df_B['ipi'] >= 20) & (df_B['year']>=2008)] # Singlet, B type
mod_ipi_SB= smf.ols('ipi~year',data=df_SB).fit()
mod_freq_SB = smf.ols('freq~year',data=df_SB).fit()

#plt.savefig('final-figs/FIG_decade_kcheck.png',dpi=600)
