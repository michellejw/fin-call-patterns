...
bathyfile = 'datasets/callpatternsbathy2.csv'
#endbathyfile = 'datasets/endeavour_large.grd'
#end_x = netcdf(endbathyfile,'r').variables['x'][::-1]
#end_y = netcdf(endbathyfile,'r').variables['y'][::-1]
#end_z = netcdf(endbathyfile,'r').variables['z'][::-1]
#dimension = netcdf(endbathyfile,'r').variables['dimension'][::-1]
#spacing = netcdf(endbathyfile,'r').variables['spacing'][::-1]
...
pts_CI = m.scatter(xsta[np.array(net=='Keck')],ysta[np.array(net=='Keck')],
       s=80,color=colvec[2], edgecolor='darkslategray',label='Keck')    
       
pts_CI = m.scatter(xsta[np.array(net=='ONC')],ysta[np.array(net=='ONC')],
       s=80,color=colvec[3], edgecolor='darkslategray',label='ONC')   
       
pts_CI = m.scatter(xsta[np.array(net=='Axial')],ysta[np.array(net=='Axial')],
       s=80,color=colvec[4], edgecolor='darkslategray',label='Axial')      

pts_CI = m.scatter(xsta[np.array(net=='Colza')],ysta[np.array(net=='Colza')],
       s=80,color=colvec[6], edgecolor='darkslategray',label='Colza')
...
fig1.savefig('CallPatterns_map3.png',dpi=300)
